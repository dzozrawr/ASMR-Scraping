...
Shader "Example/URPLitTesselationSpiralingShader"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float _Weight;
                ...
                half4 LitPassFragment(Varyings input): SV_Target
                {
                   ...
                   // half3 viewDirTS = input.viewDirTS;
                    ...
                    InitializeStandardLitSurfaceData(input.uv, surfaceData);    //u ovoj metodi se koristi _BumpMap, surfaceData je povratna vrednost
            ...
            Varyings vert(Attributes input)
            {
              ...
              //  float radius = _Radius*dP*100 + _Deviation * max(0, - (y - _MeshTop));
                float radius =dP+  _Radius*fromInitialPos.z*0.01 ;//+ _Deviation * max(0, - (y - _MeshTop));
                float length = 2 * 3.14 * (radius - _Deviation * max(0, - (y - _MeshTop)) / 2);
                float r = dP / max(0, length);
                //float a = 2 * r * 3.14*dP*100;
                float a = 2 * r * 3.14;
              ...
              // float oldY=v0.y;
                float3 fromCenter = v0 - cycleCenter;
              //  fromCenter.y=oldY;
                ...
                v0.y=fromCenter.y*c-fromCenter.z*s;
                v0.z=-fromCenter.y*s+fromCenter.z*c;

               // v0.y=fromCenter.y*s-fromCenter.z*c;
               // v0.z=fromCenter.z*c-fromCenter.y*s;

              //  v0.z=radius*c;
               // v0.y=radius*s;
                v0.xyz+=cycleCenter;    //this makes the the whole stripe move translatory
              //  v0.xyz = mul(rot_mat, fromCenter) + cycleCenter;
                
                input.positionOS.xyz = v0;
                
                
                output.positionCS = mul(UNITY_MATRIX_MVP, float4(input.positionOS.xyz, 1.0));   //this is where all the vertex changes are assigned
              ...
              //  UNITY_INITIALIZE_VERTEX_OUTPUT_STEREO(output);
                ...
                // already normalized from normal transform to WS.
               // output.normalWS = normalInput.normalWS;
                if(input.normalOS.y==0){
                     output.normalWS=float3(input.normalOS.x,0.01,input.normalOS.z);
...
#define Interpolate2(fieldName1, fieldname2) v.fieldName1 = \
				patch[0].fieldname2 * barycentricCoordinates.x + \
				patch[1].fieldname2 * barycentricCoordinates.y + \
				patch[2].fieldname2 * barycentricCoordinates.z;
            
            [UNITY_domain("tri")]
            Varyings domain(TessellationFactors factors, OutputPatch < ControlPoint, 3 > patch, float3 barycentricCoordinates: SV_DomainLocation)
            {
                Attributes v;
                Interpolate2(positionOS, vertex)

                    // DomainPos(texcoord)
                    Interpolate2(texcoord, uv)


                    Interpolate2(color, color)
                
                    //DomainPos(normalOS)
                    Interpolate2(normalOS, normal)
